...
from myblog.models import Post, Category, Link, Comment, Topic
...
class AdminTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        topic = Topic(name='test', category=category) 
        post = Post(title='Hello', subtitle='hi', category=category, topic=topic, body='Blah...')
    ...
    def test_new_topic(self):
        response = self.client.get(url_for('admin.new_topic'))
        data = response.get_data(as_text=True)
        self.assertIn('New Topic', data)

        response = self.client.post(url_for('admin.new_topic'), data=dict(name='Tech'), follow_redirects=True)
        data = response.get_data(as_text=True)
        self.assertIn('Topic created.', data)
        ...
        post = Post(title='Post Title', category=category, topic=topic)
        db.session.add(post)
        db.session.commit()
        response = self.client.get(url_for('blog.show_topic', topic_id=1))
        data = response.get_data(as_text=True)
        self.assertIn('Post Title', data)

    def test_edit_topic(self):
        """response = self.client.post(url_for('admin.edit_topic', topic_id=1),
                                    data=dict(name='Default edited'), follow_redirects=True)
        data = response.get_data(as_text=True)
        self.assertNotIn('Topic updated.', data)
        self.assertIn('Default', data)
        self.assertNotIn('Default edited', data)
        self.assertIn('You can not edit the default topic', data)""" 
        ...
        response = self.client.get(url_for('admin.edit_topic', topic_id=2))
        data = response.get_data(as_text=True)
        self.assertIn('Edit Topic', data)
        self.assertIn('Tech', data)

        response = self.client.post(url_for('admin.edit_topic', topic_id=2),
                                    data=dict(name='Life'), follow_redirects=True)
        data = response.get_data(as_text=True)
        self.assertIn('Topic updated.', data)
        self.assertIn('Life', data)
        self.assertNotIn('Tech', data)

    def test_delete_topic(self):
        topic = Topic(name='Tech')
        post = Post(title='test', topic=topic)
        db.session.add(topic)
        db.session.add(post)
        db.session.commit()

        response = self.client.get(url_for('admin.delete_topic', topic_id=1), follow_redirects=True)
        data = response.get_data(as_text=True)
        self.assertNotIn('Topic deleted.', data)
        self.assertIn('405 Method Not Allowed', data)

        '''response = self.client.post(url_for('admin.delete_topic', topic_id=1), follow_redirects=True)
        data = response.get_data(as_text=True)
        self.assertIn('You can not delete the default topic.', data)
        self.assertNotIn('Topic deleted.', data)
        self.assertIn('Default', data)'''

        response = self.client.post(url_for('admin.delete_topic', topic_id=2), follow_redirects=True)
        data = response.get_data(as_text=True)
        self.assertIn('Topic deleted.', data)
        #self.assertIn('Default', data)
        self.assertNotIn('Tech', data)
    ...
    def test_edit_link(self):
        ...
        self.assertIn('https://github.com/snytomic', data)
    ...
    def test_manage_topic_page(self):
        response = self.client.get(url_for('admin.manage_topic'))
        data = response.get_data(as_text=True)
        self.assertIn('Manage Topics', data)
